...
def get_monday_of_week(year, week):
    ...
    # Assume the time is at midnight and UTC
    dt_utc = pytz.timezone('Europe/Brussels').localize(dt)

    # Convert to local timezone (e.g., system local)
    dt_local = dt_utc.astimezone(pytz.utc)
    return dt_local
